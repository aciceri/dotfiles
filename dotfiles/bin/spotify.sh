...
if [ ! $(command -v amixer playerctl spotify | wc -l) = 3 ]
then
    echo "Error: this script requires playerctl and amixer installed and inside your \$PATH"
    exit 1
fi

is_spotify_running() {
    [ "$(ps -A | grep ' spotify$')" = "" ] && return 1 || return 0
}
